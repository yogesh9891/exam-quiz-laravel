        ...
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
            ...
            @endif
             <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                <a href="{{route('teachers.index',$school->id)}}" class="btn btn-warning ">Back </a>&nbsp; &nbsp;  Edit Teacher 
                 <span class="d-inline float-right">{{$school->school_id}} - {{$school->name}}</span>
              </h2>
                
        <form action="{{route('teachers.update',['school_id'=>$school->id,'teacher'=>$user->id])}}" method="post">
          @method('put')
            @csrf
      <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
        <div class="row">
              ...
              <div class="mb-4 col-md-6">
                  ...
                  <input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none" placeholder="Enter Teacher Name"  required   value="{{ old('name',$user->name) }}" name="name">
                  @error('name') <span class="text-red-500">{{ $message }}</span>@enderror
              </div>
              <div class="mb-4 col-md-6">
                  <label for="exampleFormControlInput2" class="block text-gray-700 text-sm font-bold mb-2"> Email :</label>
                  <input type="email" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="exampleFormControlInput2" placeholder="Enter Email" readonly=""  value="{{ old('email',$user->email) }}" name="email">
                  @error('email') <span class="text-red-500">{{ $message }}</span>@enderror
              </div>
               <div class="mb-4 col-md-6">
                  <label for="exampleFormControlInput3" class="block text-gray-700 text-sm font-bold mb-2">Phone :</label>
                  <input type="number" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="exampleFormControlInput3" placeholder="Enter Teacher phone" value="{{ old('phone',$user->phone) }}" name="phone" >

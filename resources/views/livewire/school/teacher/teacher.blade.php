...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
            ...
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Manage Teacher &nbsp;
                <button wire:click="create()" class="bg-blue-500 hover:bg-blue-700 my-3 px-4 py-2 rounded smal-texxt text-white">Create New Teacher</button>
                
            </h2>
            @if($isOpen)
                @if($updateMode)
                    @include('livewire.teacher.edit')                
                @else
                    @include('livewire.teacher.create')
                @endif
            @endif

    ...
    public function updated($name,$value)
{
    ...
    // dd($this->branches);
        ...
        // $thi->filter['tree'] = null;
         ...
         $query = [];
         $filter = $this->filter;
        	      $this->found_message ='Loading...';

         $students = StudentAssigned::with('assigned_paper')->orderBy('created_at','desc');
         ...
         // dd($this->subject_id,$this->category_id,$this->branch_id,$this->twig_id,$this->leaf_id,$this->vein_id);
           ...
           //      'template' , function($query) use ($filter){
                   ...
                   // $template  = $query->get();
                     ...
                     if(!empty($filter["tree"])){
                        ...
                        $students->whereHas(
            				  'assigned_paper.template' , function($query) use ($filter){
           ...
           //          // dd($query->get());
                     if(!empty($filter["start_date"])){
                           ...
                           $students->where('updated_at','>=', $filter["start_date"]);
                    
                        
                    } 
                      if(!empty($filter["end_date"])){
                    
                               $students->where('updated_at','<=', $filter["end_date"]);
        ...
        $this->objects = $students->get()->groupBy(['assigned_paper.question_paper.number','assigned_paper.template.title','class.class.name','section.section.name']);
    ...
    public function render()
    {
        ...
        return view('livewire.reports.paper');
    }
}
